...
import { FaPencilAlt, FaThumbsDown, FaThumbsUp, FaTrash } from "react-icons/fa";
...
const AllDonations = () => {
    ...
    const [myPet, refetch] = useDonation();
    ...
    const data = useMemo(() => {
        ...
        const dataWithSerialNumber = myPet.map((pet, index) => ({
            ...
            _id: pet._id,
            CampaignStatus:Boolean(pet.CampaignStatus)
    ...
    const handleDelete = (_id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axiosPublic.delete(`/Campaigns/${_id}`)
                    .then(res => {
        ...
        .then(res=>{
            ...
            if(res.data.modifiedCount>0){
                ...
                Swal.fire({
                    position: "top-end",
                    icon: "success",
                    title: "You have successfully made the campaign running",
    ...
    const handlePaused = (_id) => {
        // Handle adopted action for the row
        console.log("Adopted clicked for row:", _id);
        axiosPublic.patch(`/Campaigns/paused/${_id}`)
        .then(res=>{
            console.log(res.data);
            if(res.data.modifiedCount>0){
                refetch();
                Swal.fire({
                    position: "top-end",
                    icon: "success",
                    title: "You have successfully paused the campaign",
                    showConfirmButton: false,
                    timer: 1500
                  });
            }
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: ( {row} ) =>{
                ...
                console.log(row); // Add this line to log the value
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div className="flex flex-col gap-2">
                    <Link to={`/UpdateCampaign/${row.original._id}`}>
                    ...
                    {
                        row.original.CampaignStatus===true ? <btn className="btn" onClick={() => handlePaused(row.original._id)} title="Adopted">
                        <FaThumbsDown />Make Paused
                    </btn >: <btn className="btn" onClick={() => handleRunning(row.original._id)} title="Adopted">
                        <FaThumbsUp /> Make Running
                    </btn >
                    }
                </div>
            ),
    ...
    return (
        <div>
            <h1 className="text-3xl font-extrabold text-center">All Donation Campaigns</h1>
